    ...
    <main className="mx-auto max-w-6xl px-3 sm:px-4 py-5 sm:py-6 w-full min-w-0">
      ...
      <section className="rounded-xl border border-white/10 bg-[#0d141b] p-4 sm:p-5 mb-5">
        ...
        <div className="mt-4 grid gap-2.5 sm:gap-3 sm:grid-cols-2 lg:grid-cols-4">
          {Object.entries(reference.specifications || {}).slice(0, 12).map(([k, v]) => <div key={k} className="rounded-lg border border-white/[0.06] bg-white/[0.02] p-3"><div className="text-[10px] text-white/30">{k}</div><div className="mt-1 text-xs font-mono text-white/75 break-words">{specValue(v)}</div></div>)}
        </div>
      </section>
      ...
      <section className="space-y-3">
        ...
        {alternatives.length === 0 ? <div className="rounded-xl border border-white/10 bg-[#0d141b] p-8 text-center text-sm text-white/40">No hay suficientes datos comparables en el Knowledge Core. Esto no significa que no exista un equivalente; significa que la evidencia disponible no alcanza el umbral técnico.</div> : alternatives.map((a) => {
          ...
          return <article key={a.id} className="rounded-xl border border-white/10 bg-[#0d141b] p-4 sm:p-5">
            ...
            <div className="mt-4 flex flex-wrap gap-1.5 sm:gap-2">{Object.entries(a.specifications || {}).slice(0, 8).map(([k,v]) => <span key={k} className="rounded bg-white/[0.04] border border-white/10 px-2 py-1 text-[10px] text-white/55"><span className="text-white/30">{k}:</span> {specValue(v)}</span>)}</div>
