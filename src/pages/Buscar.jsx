  ...
  return (
    <div className="ip-shell grid-bg">
      <header className="ip-header px-3 sm:px-5 py-3 ip-mobile-safe-top">
        <form onSubmit={submit} className="relative mx-auto max-w-[1120px] flex items-center gap-2 ip-card pl-2.5 sm:pl-3 pr-1.5 py-1.5 shadow-[0_16px_40px_-30px_rgba(0,0,0,.9)] focus-within:border-[#ea580c]/60 focus-within:ring-4 focus-within:ring-[#ea580c]/10 transition-all">
          <Link to="/" className="text-white/50 hover:text-white">
            <ArrowLeft className="w-4 h-4" />
          </Link>
          <span className="w-8 h-8 rounded-xl bg-[#ea580c]/10 border border-[#ea580c]/10 flex items-center justify-center shrink-0"><Search className="w-4 h-4 text-[#ea580c]" /></span>
          <input
            value={input}
          ...
          />
          <button type="submit" className="bg-[#ea580c] hover:bg-[#4f8fc7] text-[#0a0e12] text-xs sm:text-sm font-semibold px-3.5 sm:px-5 py-2.5 rounded-xl transition-colors shrink-0">
            {t.search}
          </button>
        ...
        {trialNotice === 'last' && (
          <div className="mx-auto mt-2 max-w-2xl px-2 text-center text-xs text-[#ea580c]">Te queda 1 prueba gratuita.</div>
        )}
        {trialNotice === 'limit' && (
          <div className="ip-card mx-auto mt-3 max-w-md p-4 text-center shadow-xl">
            ...
            <button type="button" onClick={() => navigate(`/login?returnTo=${encodeURIComponent('/buscar')}`)} className="mt-4 w-full rounded-lg bg-[#ea580c] px-4 py-2.5 text-sm font-semibold text-[#080d12]">Registrarme gratis</button>
          </div>
        )}
      </header>
      ...
      <main className="px-3 sm:px-5 py-6 sm:py-8 max-w-[1120px] mx-auto w-full">
        <div className="flex items-center justify-between gap-2 mb-5 sm:mb-6 pb-4 border-b border-white/[0.06]">
          ...
          <button
            ...
            className="flex items-center gap-1.5 text-white/60 hover:text-white hover:border-[#ea580c]/35 hover:bg-white/[0.025] text-xs border border-white/10 rounded-xl px-3 py-2 transition-colors"
          >
        ...
        {!q && !area ? (
          ...
          <div className="rounded-2xl border border-amber-400/20 bg-amber-400/[0.04] p-5 sm:p-6 text-center">
            ...
            <button onClick={onReset} className="mt-3 text-xs text-[#ea580c] hover:underline">{t.retry}</button>
          </div>
        ) : (
          <div className="space-y-6">
            {kcLoading ? (
