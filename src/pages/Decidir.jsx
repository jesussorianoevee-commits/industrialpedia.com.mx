        ...
        <form onSubmit={decide} className="ip-card p-4 sm:p-5">
          <div className="grid gap-4 md:grid-cols-[1fr_2fr] min-w-0">
            <label className="block">
              ...
              <input value={family} onChange={(e) => setFamily(e.target.value)} placeholder="Ej. sensor, cilindro, motor" className="mt-2 w-full rounded-lg border border-white/10 bg-[#080d12] px-3 py-2.5 text-sm text-white outline-none focus:border-[#2563eb]/50" />
            </label>
            <div>
              <div className="flex items-center justify-between">
                <span className="text-[10px] uppercase tracking-wider text-white/35">{t.requirements}</span>
                <button type="button" onClick={addRequirement} className="inline-flex items-center gap-1 text-[10px] text-[#2563eb] hover:text-white"><Plus className="h-3.5 w-3.5" /> {t.add}</button>
              </div>
              <div className="mt-2 space-y-2">
                {requirements.map((row, index) => (
                  <div key={index} className="grid grid-cols-1 sm:grid-cols-[minmax(0,1fr)_minmax(0,1fr)_auto] gap-2">
                    <input value={row.key} onChange={(e) => updateRequirement(index, 'key', e.target.value)} placeholder={t.attribute} className="min-w-0 rounded-lg border border-white/10 bg-[#080d12] px-3 py-2 text-xs text-white outline-none focus:border-[#2563eb]/50" />
                    <input value={row.value} onChange={(e) => updateRequirement(index, 'value', e.target.value)} placeholder={t.requiredValue} className="min-w-0 rounded-lg border border-white/10 bg-[#080d12] px-3 py-2 text-xs text-white outline-none focus:border-[#2563eb]/50" />
          ...
          {trialNotice === 'last' && <div className="mt-4 text-xs text-[#2563eb]">Te queda 1 prueba gratuita.</div>}
          {trialNotice === 'limit' && (
            <div className="mt-4 ip-card p-4 text-center">
              ...
              <button type="button" onClick={() => navigate('/login?returnTo=%2Fdecidir')} className="mt-3 rounded-lg bg-[#2563eb] px-4 py-2 text-sm font-semibold text-[#080d12]">Registrarme gratis</button>
            </div>
          )}
          <button type="submit" disabled={loading} className="mt-5 inline-flex w-full sm:w-auto items-center justify-center gap-2 rounded-lg bg-[#2563eb] px-5 py-2.5 text-xs font-semibold text-[#080d12] hover:bg-[#a78bfa] disabled:opacity-50">
                ...
                <article key={result.id || result.part_id || result.part_number || index} className="ip-card p-4 sm:p-5">
                  <div className="flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between">
                    <div className="min-w-0">
                      <div className="font-mono text-sm text-[#2563eb] break-all">{result.part_number || result.part_id || 'Candidato sin número de parte'}</div>
                  ...
                  {result.source_url && <a href={result.source_url} target="_blank" rel="noreferrer" className="mt-4 inline-block text-[10px] text-[#2563eb] hover:underline">{t.viewSource} →</a>}
                </article>
              );
            })}
          </section>
