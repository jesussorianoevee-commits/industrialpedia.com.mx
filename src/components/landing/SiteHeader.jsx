    ...
    <header className="ip-header">
      <div className="ip-container ip-header-inner py-2.5">
        <Link to="/" className="flex items-center gap-3 group shrink-0">
          <span className="w-9 h-9 rounded-xl border border-[#ea580c]/40 bg-[#ea580c]/10 flex items-center justify-center shadow-[0_0_24px_-10px_rgba(90,156,217,.7)]">
            <span className="w-2 h-2 rounded-full bg-[#ea580c] shadow-[0_0_12px_rgba(90,156,217,.8)]" />
          </span>
        ...
        <div className="flex items-center gap-1.5 sm:gap-2 shrink-0">
          ...
          <div className="relative">
            <button type="button" onClick={() => setLanguageOpen((v) => !v)} aria-label={t.language} title={t.language} className="ip-button-tertiary flex items-center gap-1.5 h-9 px-2.5 sm:px-3 border border-white/10 bg-white/[0.03] hover:bg-primary/10">
              <Globe className="w-4 h-4 text-[#ea580c]" /> <span className="font-semibold">{language.toUpperCase()}</span><span className="text-white/30">▾</span>
            </button>
