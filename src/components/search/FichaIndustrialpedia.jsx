...
function SpecRow({ label, value, unit, page, verified, sourceUrl }) {
  ...
  const localizedValue = localizeSpecValue(value, language);
  const text = `${localizedValue}${unit ? ` ${unit}` : ''}`;
      ...
      <div className="min-w-0 text-left sm:text-right">
        ...
        {sourceUrl && (
          <a
            ...
            target="_blank"
            rel="noreferrer"
            className="inline-flex items-center gap-1 mt-1 text-[10px] text-[#2563eb]/75 hover:text-[#2563eb]"
          >
          ...
          <div className="p-3 sm:p-5 md:p-6 space-y-4 sm:space-y-5 md:space-y-6">
            <section className="ip-card border-white/10 p-3.5 sm:p-5 md:p-6">
              <div className="flex items-start gap-4">
                ...
                <div className="min-w-0 flex-1">
                  {ficha.product_identity?.manufacturer && <div className="text-[11px] text-[#2563eb] font-medium uppercase tracking-[0.14em] truncate">{ficha.product_identity.manufacturer}</div>}
            ...
            <section className="ip-card p-4">
              <div className="flex items-center justify-between gap-3">
                ...
                {ficha.source?.url && (
                  <a href={ficha.source.url} target="_blank" rel="noreferrer" className="shrink-0 inline-flex items-center gap-1.5 text-xs text-[#2563eb] hover:underline">
