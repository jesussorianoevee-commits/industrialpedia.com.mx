      ...
      {!isAuthenticated ? (
        ...
        <form onSubmit={submit} className="mt-4 space-y-3">
          <div className="grid gap-3 sm:grid-cols-2">
            <label className="block sm:col-span-2"><span className="text-[10px] uppercase tracking-wider text-white/35">Número de parte con el que tienes experiencia</span><input value={targetPartNumber} onChange={(e) => setTargetPartNumber(e.target.value)} placeholder="Ej. ABC-123" className="mt-1.5 w-full rounded-lg border border-white/10 bg-[#0a0e12] px-3 py-2.5 font-mono text-sm text-white outline-none focus:border-[#47bcb6]/60" required /></label>
            <label className="block"><span className="text-[10px] uppercase tracking-wider text-white/35">Resultado</span><select value={outcome} onChange={(e) => setOutcome(e.target.value)} className="mt-1.5 w-full rounded-lg border border-white/10 bg-[#0a0e12] px-3 py-2.5 text-sm text-white outline-none"><option value="confirmed">Compatible / funcionó</option><option value="incompatible">Incompatible / no funcionó</option></select></label>
            <label className="block"><span className="text-[10px] uppercase tracking-wider text-white/35">Tipo de experiencia</span><select value={experienceType} onChange={(e) => setExperienceType(e.target.value)} className="mt-1.5 w-full rounded-lg border border-white/10 bg-[#0a0e12] px-3 py-2.5 text-sm text-white outline-none"><option value="field_use">Uso en campo</option><option value="installation">Instalación</option><option value="technical_review">Revisión técnica</option></select></label>
          </div>
          <label className="block"><span className="text-[10px] uppercase tracking-wider text-white/35">Nota (opcional)</span><textarea value={note} onChange={(e) => setNote(e.target.value)} maxLength={500} rows={3} placeholder="Comparte únicamente el contexto técnico relevante." className="mt-1.5 w-full resize-none rounded-lg border border-white/10 bg-[#0a0e12] px-3 py-2.5 text-sm text-white outline-none focus:border-[#47bcb6]/60" /></label>
          <button type="submit" disabled={!canSubmit || loading} className="inline-flex w-full sm:w-auto items-center justify-center gap-2 rounded-lg bg-[#47bcb6] px-4 py-2.5 text-xs font-semibold text-[#071012] hover:bg-[#62ccc6] disabled:cursor-not-allowed disabled:opacity-50">{loading ? <Loader2 className="h-4 w-4 animate-spin" /> : outcome === 'confirmed' ? <CheckCircle2 className="h-4 w-4" /> : <XCircle className="h-4 w-4" />}{loading ? 'Registrando...' : 'Registrar experiencia'}</button>
