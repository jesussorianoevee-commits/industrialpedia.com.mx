...
export default function CompatibilityCommunity({ part }) {
  ...
  return (
    <section className="ip-card p-4 sm:p-5">
      ...
      {!isAuthenticated ? (
        <div className="mt-4 rounded-lg border border-[#8b5cf6]/20 bg-[#8b5cf6]/[0.05] p-3">
          <div className="text-xs text-white/70">{t.accountRequiredCompatibility}</div>
          <Link to={`/login?returnTo=${encodeURIComponent(window.location.pathname + window.location.search)}`} className="mt-3 inline-flex items-center gap-2 rounded-lg bg-[#8b5cf6] px-3 py-2 text-xs font-semibold text-[#080d12] hover:bg-[#a78bfa]"><LogIn className="h-3.5 w-3.5" /> {t.signInToParticipate}</Link>
        </div>
      ) : (
