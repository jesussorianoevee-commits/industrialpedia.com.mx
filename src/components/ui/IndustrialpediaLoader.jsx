  ...
  return (
    <div className={fullScreen ? 'fixed inset-0 z-[9999] flex items-center justify-center overflow-hidden bg-[#080d12]' : 'flex min-h-[360px] items-center justify-center overflow-hidden'}>
      <div className="relative flex h-full w-full flex-col items-center justify-center px-4">
        {!videoFailed ? (
          <video
            className="h-auto w-auto max-h-[100svh] max-w-full select-none object-contain"
            src="/industrialpedia-loader.mp4"
            autoPlay
            muted
            playsInline
            loop
            preload="auto"
            aria-label="Industrialpedia cargando"
        ...
        <div className="pointer-events-none absolute bottom-8 left-0 right-0 flex flex-col items-center gap-2">
          ...
          <div className="h-px w-40 max-w-[60vw] overflow-hidden bg-white/10">
            <div className="h-full w-1/3 bg-[#5a9cd9] loader-progress" />
          </div>
        </div>

        <style>{`
          .loader-mascot { animation: loader-bob 1.8s ease-in-out infinite; filter: drop-shadow(0 14px 22px rgba(0,0,0,.28)); }
          ...
          @keyframes loader-bob { 0%,100% { transform: translate3d(0,0,0) scale(1); } 50% { transform: translate3d(0,-7px,0) scale(1.012); } }
          ...
          @media (prefers-reduced-motion: reduce) { .loader-mascot, .loader-dot, .loader-progress { animation: none !important; } }
        `}</style>
      </div>
    </div>
  );
